  ...
  switch (data.command) {
    ...
    case 'addAllItemsToRundown': 
      ...
      try {      
        ...
        ProfileTemplatesArray.forEach((template,index) => {
          template.itemID = String(Math.floor(Math.random() * 90000) + 10000); // generate EPOCH for each template
        });
    ...
    case 'addItemToRundown':
      ...
      try {      
        ...
        TemplateModel.itemID = String(Math.floor(Math.random() * 90000) + 10000); // Generate ID using Math Random for this rundown item (from v. 0.9.8 onwards)
